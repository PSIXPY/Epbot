...
# === ОБЪЕДИНЕНИЕ ЧАТОВ (ПОЛНОСТЬЮ РАБОЧАЯ ВЕРСИЯ) ===
...
def relay_messages(message):
    ...
    if chat_id == CHAT_A:
        target_chat = CHAT_B
        target_thread = CHAT_B_THREAD if CHAT_B_THREAD != 0 else None
    elif chat_id == CHAT_B:
    ...
    try:
        # ТЕКСТ - отправляем с подписью
        if message.text:
            bot.send_message(target_chat, f"📩 *{sender_name}*\n\n{message.text}", 
                           parse_mode="Markdown", message_thread_id=target_thread)
            logger.info(f"📝 Переслан текст")
        
        # ФОТО
        elif message.photo:
            caption = f"📩 *{sender_name}*\n\n{message.caption or ''}"
            bot.send_photo(target_chat, message.photo[-1].file_id, caption=caption,
                         parse_mode="Markdown", message_thread_id=target_thread)
        ...
        # ДОКУМЕНТЫ
            ...
            caption = f"📩 *{sender_name}*\n\n{message.caption or ''}"
            bot.send_document(target_chat, message.document.file_id, caption=caption,
                            parse_mode="Markdown", message_thread_id=target_thread)
            logger.info(f"📄 Переслан документ")
        
        # GIF
        elif message.animation:
            caption = f"📩 *{sender_name}*\n\n{message.caption or ''}"
            bot.send_animation(target_chat, message.animation.file_id, caption=caption,
                             parse_mode="Markdown", message_thread_id=target_thread)
            logger.info(f"🎞️ Переслана GIF")
        
        # СТИКЕРЫ
        elif message.sticker:
            bot.send_sticker(target_chat, message.sticker.file_id, message_thread_id=target_thread)
            bot.send_message(target_chat, f"📩 *{sender_name}* (стикер)",
                           parse_mode="Markdown", message_thread_id=target_thread)
            logger.info(f"🏷️ Переслан стикер")
        
        # ГОЛОСОВЫЕ
        elif message.voice:
            bot.send_voice(target_chat, message.voice.file_id, caption=f"📩 *{sender_name}*",
                         parse_mode="Markdown", message_thread_id=target_thread)
            logger.info(f"🎤 Переслано голосовое")
        
        # АУДИО
        elif message.audio:
            bot.send_audio(target_chat, message.audio.file_id, caption=f"📩 *{sender_name}*",
                         parse_mode="Markdown", message_thread_id=target_thread)
            logger.info(f"🎵 Переслано аудио")
        
        # ВИДЕОСООБЩЕНИЯ
        elif message.video_note:
            bot.send_video_note(target_chat, message.video_note.file_id, message_thread_id=target_thread)
            bot.send_message(target_chat, f"📩 *{sender_name}* (видеосообщение)",
                           parse_mode="Markdown", message_thread_id=target_thread)
...
def web_search(q):
    try:
        ...
        if not results:
            return None
        res = [f"• [{x.get_text()}]({x.get('href')})" for x in results if x.get('href')]
...
👑 *Админ:* /backup""", parse_mode="Markdown")
...
def wiki_cmd(m):
    ...
    if not q:
        ...
        return
    bot.reply_to(m, search_wiki(q), parse_mode="Markdown")
...
def backup_cmd(m):
    ...
    try:
        ...
        with open(fname, 'rb') as f:
            bot.send_document(m.chat.id, f, caption=f"📦 Бекап напоминаний\n\nДата: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}\nВсего: {len(data)}")
        os.remove(fname)
    except Exception as e:
        bot.reply_to(m, f"❌ {e}")
...
def remind_cmd(m):
    ...
    loc = "в этот же топик" if thread_id else "в этот чат"
...
def reminds_cmd(m):
    ...
    if not user_r:
        msg = bot.reply_to(m, "📭 Нет активных напоминаний")
        delete_after_delay(chat_id, msg.message_id, 15)
        return
    resp = "📋 *Активные напоминания в этом чате/топике:*\n\n"
    for r in user_r:
        if r.get("daily"):
        ...
        resp += f"🆔 `{r['id']}` — {p}\n   📝 {r['text'][:50]}\n\n"
    msg = bot.send_message(chat_id, resp, parse_mode="Markdown", message_thread_id=thread_id)
        ...
        for i, r in enumerate(reminders):
            if r["id"] == rid:
                if r.get("chat_id") != chat_id:
                    msg = bot.reply_to(m, f"❌ Напоминание {rid} не найдено")
                    delete_after_delay(chat_id, msg.message_id, 15)
                    return
                if thread_id and r.get("thread_id") != thread_id:
                    msg = bot.reply_to(m, f"❌ Напоминание {rid} не в этом топике")
                    delete_after_delay(chat_id, msg.message_id, 15)
                    return
                if "_timer" in r:
                    try:
                ...
                reminders.pop(i)
                save_reminders(reminders)
                msg = bot.reply_to(m, f"✅ Напоминание {rid} удалено")
                delete_after_delay(chat_id, msg.message_id)
                return
        msg = bot.reply_to(m, f"❌ Напоминание {rid} не найдено")
        delete_after_delay(chat_id, msg.message_id, 15)
...
def tr_cmd(m):
    ...
    parts = m.text.split()
    if len(parts) < 2:
        status = "✅ Включён" if is_tr_enabled(chat_id) else "❌ Выключен"
        bot.reply_to(m, f"🌐 *Переводчик*\nСтатус: {status}\n\n`/т on` — включить\n`/т off` — выключить", parse_mode="Markdown")
        return
    if parts[1].lower() == "on":
        set_tr_enabled(chat_id, True)
        bot.reply_to(m, "✅ *Переводчик включён!*", parse_mode="Markdown")
    elif parts[1].lower() == "off":
        set_tr_enabled(chat_id, False)
        bot.reply_to(m, "❌ *Переводчик выключен*", parse_mode="Markdown")
...
def auto_tr(m):
    ...
    if not is_tr_enabled(chat_id):
        return
    if m.from_user.id == bot.get_me().id or m.text.startswith('/') or m.text.startswith('📩') or m.text.startswith('📨'):
        return
    text = m.text.strip()
...
def inline_q(q):
    try:
        ...
        try:
            ...
            if target.isdigit():
                tid = int(target)
                tname = f"Пользователь {target}"
            else:
                res = types.InlineQueryResultArticle(
                    id="err",
                    title="❌ Пользователь не найден",
                    description=f"{target} не найден",
                    input_message_content=types.InputTextMessageContent(f"❌ Пользователь {target} не найден")
                )
...
def secret_read(c):
    ...
    if mid not in secret_messages:
        bot.answer_callback_query(c.id, "❌ Сообщение не найдено", show_alert=True)
        return
    data = secret_messages[mid]
    if c.from_user.id != data["target_id"]:
        bot.answer_callback_query(c.id, "❌ Это сообщение не для вас", show_alert=True)
        return
    if time.time() > data["expires"]:
        bot.answer_callback_query(c.id, "❌ Срок действия истёк", show_alert=True)
...
def channel_react(m):
    ...
    try:
        ...
        logger.info(f"🔥 Реакция на пост {m.message_id}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    logger.info(f"Чат A: {CHAT_A}, Чат B: {CHAT_B}")
