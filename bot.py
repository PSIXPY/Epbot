...
def relay_messages(message):
    ...
    if chat_id == CHAT_A:
        try:
            # ТЕКСТ - отправляем новое сообщение с подписью
            if message.text:
                bot.send_message(CHAT_B, f"📩 *{sender_name}*\n\n{message.text}", 
                               parse_mode="Markdown", message_thread_id=CHAT_B_THREAD)
            # ФАЙЛЫ, GIF, СТИКЕРЫ, ФОТО, ВИДЕО - пересылаем оригинал + добавляем подпись
            else:
                bot.forward_message(CHAT_B, CHAT_A, message.message_id, 
        ...
        try:
            ...
            if message.text:
                bot.send_message(CHAT_A, f"📩 *{sender_name}*\n\n{message.text}", 
                               parse_mode="Markdown")
            # ФАЙЛЫ - пересылаем оригинал + подпись
            else:
                bot.forward_message(CHAT_A, CHAT_B, message.message_id)
                bot.send_message(CHAT_A, f"📩 *{sender_name}*", parse_mode="Markdown")
...
def clear_history(message):
    ...
    if user_id in user_histories:
        ...
        bot.reply_to(message, "🗑️ История очищена!", parse_mode="Markdown")
    else:
        bot.reply_to(message, "📭 Нет сохранённой истории", parse_mode="Markdown")
...
if __name__ == "__main__":
    ...
    logger.info("✅ Файлы/GIF/стикеры → forward_message")
