...
# === ДИАГНОСТИЧЕСКИЙ ХЕНДЛЕР - ЛОВИТ ВСЁ ===
@bot.message_handler(func=lambda m: True)
def debug_echo(message):
    print(f"🔴🔴🔴 ДИАГНОСТИКА: {message.text} | Чат: {message.chat.id} | Тип: {message.chat.type}")
    print(f"🔴 От пользователя: {message.from_user.id}, имя: {message.from_user.first_name}")
    
    # Отвечаем на ВСЕ сообщения в ЛС
    if message.chat.type == 'private':
        bot.reply_to(
            message, 
            f"✅ *Бот видит сообщение!*\n\n"
        ...
        # Если это backup - пробуем создать файл
        if message.text and message.text.lower() in ['/backup', 'бекап', 'backup']:
            print("🔵 ВЫЗВАН БЕКАП из диагностического хендлера")
            try:
                with open('test.txt', 'w') as f:
                    f.write(f"Тест бекапа\nВремя: {datetime.now()}\nАдмин: {ADMIN_ID}")
                with open('test.txt', 'rb') as f:
                    bot.send_document(message.chat.id, f, caption="✅ *ТЕСТОВЫЙ БЕКАП*", parse_mode="Markdown")
                os.remove('test.txt')
                print("🔵 Тестовый файл отправлен")
            except Exception as e:
                bot.send_message(message.chat.id, f"❌ Ошибка бекапа: {e}")
                print(f"🔴 Ошибка бекапа: {e}")
        
        # Если это restore
        elif message.text and message.text.lower() in ['/restore', 'восстановление', 'restore']:
            bot.reply_to(
                message,
